...
function CRMSystem() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(customersRef, (snapshot) => {
      const loadedCustomers = snapshot.docs.map(doc => ({ ...doc.data(), id: doc.id }));
  ...
  const handleDeleteCustomer = (id) => {
    setConfirmDialog({
      ...
      onConfirm: async () => {
        if (user && db) {
          await deleteDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'customers', id.toString()));
        }
    ...
    setConfirmDialog({
      ...
      onConfirm: async () => {
        if (user && db) {
          ...
          if (customer) {
            const updatedCustomer = { ...customer, purchases: customer.purchases.filter(p => p.id !== purchaseId) };
            await setDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'customers', customerId.toString()), updatedCustomer);
  ...
  const handleSaveCustomer = async (e) => {
    ...
    if (editingCustomer) {
      ...
      await setDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'customers', editingCustomer.id.toString()), updatedCustomer);
    } else {
      ...
      await setDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'customers', newId), newCustomer);
    }
    setIsCustomerModalOpen(false);
  };

  const handleSaveSessions = async (e) => {
    e.preventDefault();
    if (!user || !editingCustomer || !db) return;
          ...
          onConfirm: async () => {
            if (user && db) {
              for (const newCust of newCustomers) {
                await setDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'customers', newCust.id), newCust);
              }
            }
            setConfirmDialog({ isOpen: false, message: '', onConfirm: null, confirmText: '確定', confirmStyle: 'danger' });
          }
                  ...
                  onConfirm: async () => {
                    if (user && db) {
                      for (const c of customers) {
                        await deleteDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'customers', c.id.toString()));
                      }
                    }
                    setConfirmDialog({ isOpen: false, message: '', onConfirm: null, confirmText: '確定', confirmStyle: 'danger' });
                  }
